...
static bool invuse; //Need special case to allow using inventory
...
void PortableAction(int state, int action)
{
	...
	else
	{
		...
		case PORT_ACT_INVNEXT:
			key = key_invright;
			break;
	    case PORT_ACT_INVSLOT0:
	    case PORT_ACT_INVSLOT1:
	    case PORT_ACT_INVSLOT2:
	    case PORT_ACT_INVSLOT3:
	    case PORT_ACT_INVSLOT4:
	    case PORT_ACT_INVSLOT5:
			key = key_invslot0 + (action - PORT_ACT_INVSLOT0);
			break;
...
{
    LOGI("PortableMouseAbs %f %f",x,y);
	absx = x;
	absy = y;
	//SDL_SendMouseMotion(NULL,1,x,y);
}


void PortableMouseButton(int state, int buttons)
{
...
extern boolean st_displaypopup;


//To find the Y/N message
extern int			messageToPrint;
extern boolean			messageNeedsInput;
touchscreemode_t PortableGetScreenMode()
{
	if ((menuactive || paused || !usergame))
	{
...
void PortableAutomapControl(float zoom, float x, float y)
{
	...
	am_pan_x += x;
	am_pan_y += y;
	LOGI("am_pan_x = %f, am_zoom = %f", am_pan_x, am_zoom );
}
